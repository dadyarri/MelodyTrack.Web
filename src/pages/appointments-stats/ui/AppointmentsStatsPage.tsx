...
import {
  analyticsQueryKeys,
  type AppointmentHourAnalytics,
  type AppointmentLoadByDay,
  type BurnedClientAnalytics,
  dashboardApi,
  type TeacherAppointmentsAnalytics,
...
export function AppointmentsStatsPage() {
  const controller = useDashboardDateRangeQuery({
    getQueryKey: ({ timezone, dateRange }) => analyticsQueryKeys.appointments(timezone, dateRange[0], dateRange[1]),
    ...
    >
      ...
      <div data-onboarding-id="appointments-stats-summary">
        <SummaryGrid>
          ...
          <SummaryCard
            title={<InfoLabel label="Занято часов" tooltip="Сумма длительностей запланированных, проведенных и сгоревших записей." />}
            value={formatHours(query.data?.takenHours)}
          />
          <SummaryCard
            title={<InfoLabel label="Доступно часов" tooltip="Сумма рабочих часов преподавателей по настроенному графику и отпускам." />}
            value={formatHours(query.data?.availableHours)}
          />
          <SummaryCard
            title={<InfoLabel label="Загрузка" tooltip="Доля занятого времени от доступного рабочего времени преподавателей." />}
            value={formatPercent(query.data?.loadPercentage)}
          />
          <SummaryCard
            title={<InfoLabel label="Проведено часов" tooltip="Сумма длительностей только проведенных записей." />}
            value={formatHours(query.data?.workedHours)}
          />
          <SummaryCard title="Свободно часов" value={formatHours(query.data?.freeHours)} />
          <SummaryCard
            title={
              <InfoLabel
                label="Средне проведено на преподавателя"
                tooltip="Среднее число проведенных записей на одного активного преподавателя за выбранный период."
              />
            }
            value={formatDecimal(query.data?.averageCompletedAppointmentsPerTeacher)}
          />
          <SummaryCard
            title={
              <InfoLabel
                label="Средний интервал между занятиями"
                tooltip="Средний положительный интервал между соседними проведенными занятиями одного преподавателя."
              />
            }
            value={formatHours(query.data?.averageGapBetweenServicesHours)}
          />
          <SummaryCard
            title={<InfoLabel label="Выручка" tooltip="Выручка по проведенным и сгоревшим записям в выбранном периоде." />}
            value={formatMoney(query.data?.totalRevenue)}
          />
          ...
          <SummaryCard
            title={
              <InfoLabel
                label="Отменено"
                tooltip="Отмененные записи не входят в фактическую выручку и по умолчанию не занимают слот в загрузке."
              />
            }
            value={query.data?.cancelledAppointmentsCount ?? 0}
          />
          <SummaryCard
            title={
              <InfoLabel label="Сгорело" tooltip="Сгоревшие записи считаются в выручке и в занятом времени, но не в отработанных часах." />
            }
            value={query.data?.burnedAppointmentsCount ?? 0}
          />
          ...
          <SectionCard title="Структура статусов">
            <StatsDonutChart
              items={query.data?.statuses.map((item, index) => ({
                ...
                tooltip: `${statusLabels[item.status]}: ${String(item.count)}${item.share == null ? "" : ` (${formatPercent(item.share)})`}`,
            ...
            columns={[
              ...
              { title: "Уникальных клиентов", dataIndex: "uniqueClientsCount" },
              {
                title: (
                  <InfoLabel
                    label="Рабочих дней"
                    tooltip="Дни с доступными рабочими часами по графику преподавателя в выбранном периоде."
                  />
                ),
                dataIndex: "workingDaysCount",
              },
              ...
              {
                title: (
                  <InfoLabel label="Занято" tooltip="Часы преподавателя, занятые запланированными, проведенными и сгоревшими записями." />
                ),
              ...
              {
                title: <InfoLabel label="Простой" tooltip="Доля доступного времени преподавателя, которая не была занята записями." />,
                dataIndex: "downtimeShare",
              ...
              {
                title: <InfoLabel label="Выручка / отработанный час" tooltip="Выручка, деленная на часы по проведенным записям." />,
                dataIndex: "revenuePerWorkedHour",
                render: (value?: number | null) => (value == null ? "—" : formatMoney(value)),
              },
              {
                title: (
                  <InfoLabel
                    label="Выручка / занятый час"
                    tooltip="Выручка, деленная на все занятые часы по запланированным, проведенным и сгоревшим записям."
                  />
                ),
                dataIndex: "revenuePerOccupiedHour",
                render: (value?: number | null) => (value == null ? "—" : formatMoney(value)),
              },
              {
                title: (
                  <InfoLabel
                    label="Проведено в день"
                    tooltip="Среднее количество проведенных записей на один рабочий день преподавателя."
                  />
                ),
                dataIndex: "averageCompletedAppointmentsPerWorkingDay",
              ...
              },
              {
                title: (
                  <InfoLabel
                    ...
                    tooltip="Средний положительный интервал между соседними проведенными занятиями преподавателя."
                  />
                ),
                dataIndex: "averageGapBetweenServicesHours",
                render: formatHours,
              },
              {
                title: (
                  <InfoLabel
                    label="Топ услуг"
                    tooltip="Пять самых частых услуг преподавателя по количеству проведенных записей. Доля считается от всех проведенных записей преподавателя."
                  />
                ),
                render: (_, row) => (
          ...
          <SectionCard title="Часы пик">
            <HoursTable
              ...
              data={query.data?.hours
                ...
                .sort((a, b) => b.appointmentsCount - a.appointmentsCount || (b.loadPercentage ?? 0) - (a.loadPercentage ?? 0))
                .slice(0, 8)}
            />
          </SectionCard>
        ...
        <SectionCard title="Загрузка по дням">
          <Table<AppointmentLoadByDay>
            ...
            columns={[
              ...
              {
                title: <InfoLabel label="Оказано услуг" tooltip="Количество проведенных записей за день." />,
                dataIndex: "servicesProvidedCount",
              },
              {
                title: "Статусы",
                render: (_, row) => (
              ...
              { title: "Клиентов", dataIndex: "uniqueClientsCount" },
              {
                title: (
                  <InfoLabel
                    label="Клиентов по проведенным"
                    tooltip="Количество уникальных клиентов, у которых в этот день была хотя бы одна проведенная запись."
                  />
                ),
                dataIndex: "completedUniqueClientsCount",
              },
              { title: "Выручка", dataIndex: "revenue", render: (value: number) => formatMoney(value) },
...
const hourColumns = [
  ...
  title: ReactNode;
  dataIndex?: keyof AppointmentHourAnalytics;
  render?: (value: never, row: AppointmentHourAnalytics) => ReactNode;
}>;
