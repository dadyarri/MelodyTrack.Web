    ...
    >
      ...
      <div data-onboarding-id="clients-stats-summary">
        <StatsCardGrid>
          ...
          <SummaryCard
            title={titleWithInfo("Доля потерянных", "Потерянные клиенты / клиенты, у которых в истории была хотя бы одна запись.")}
          ...
          <SummaryCard
            title={titleWithInfo(
              ...
              "Среднее число дней между первой и последней записью клиента. Для клиентов с одной записью длина обучения равна 0 дней.",
            )}
            value={formatDays(query.data?.averageClientLifetimeDays)}
          />
          <SummaryCard
            title={titleWithInfo("VIP", "Клиенты из верхних 10% по LTV среди всех клиентов с ненулевой выручкой.")}
          ...
          <SectionCard
            title={titleWithInfo(
              ...
              <Flex vertical gap={8}>
                ...
                <ul>
                  <li>
                    <b>VIP</b> — верхние 10% по LTV;
                  </li>
                  <li>
                    <b>Регулярные</b> — минимум 4 проведенные записи за последние 90 дней;
                  </li>
                  <li>
                    <b>Разовые</b> — ровно 1 проведенная запись за всю историю;
                  </li>
                  <li>
                    <b>Новые</b> — созданы в выбранном периоде;
                  </li>
                  <li>
                    <b>Под риском</b> — давно не приходили относительно своего среднего интервала;
                  </li>
                  <li>
                    <b>Потерянные</b> — не было записей последние 30 дней при наличии истории;
                  </li>
                  <li>
                    <b>Должники</b> — выручка больше оплат.
                  </li>
                </ul>
              </Flex>,
            )}
          ...
          <SectionCard
            title={titleWithInfo(
              ...
              <Flex vertical gap={8}>
                ...
                <ul>
                  <li>
                    <b>Recency</b> — как давно клиент был в последний раз;
                  </li>
                  <li>
                    <b>Frequency</b> — сколько проведенных записей у клиента в выбранном периоде;
                  </li>
                  <li>
                    <b>Monetary</b> — сколько выручки клиент принес в выбранном периоде по проведенным и сгоревшим записям.
                  </li>
        ...
        <Table<ClientRfmAnalytics>
          ...
          columns={[
            ...
            },
            {
              title: titleWithInfo(
                "F",
                "Оценка Frequency от 1 до 5. 5 получают клиенты с наибольшим числом проведенных записей за период.",
              ),
              dataIndex: "frequencyScore",
            },
            {
              title: titleWithInfo("M", "Оценка Monetary от 1 до 5. 5 получают клиенты с наибольшей выручкой за период."),
              dataIndex: "monetaryScore",
            },
            {
              title: titleWithInfo(
                "RFM",
...
function SegmentTags({ row }: { row: ClientAnalytics }) {
  const tags = [
    row.isVip ? (
      <Tag key="vip" color="gold">
        VIP
      </Tag>
    ) : null,
    row.isRegular ? (
      <Tag key="regular" color="blue">
        Регулярный
      </Tag>
    ) : null,
    ...
    row.isReturned ? (
      <Tag key="returned" color="cyan">
        Вернувшийся
      </Tag>
    ) : null,
    row.isAtRisk ? (
      <Tag key="risk" color="orange">
        Риск
      </Tag>
    ) : null,
    row.isLost ? (
      <Tag key="lost" color="red">
        Потерян
      </Tag>
    ) : null,
    row.isDebtor ? (
      <Tag key="debtor" color="volcano">
        Должник
      </Tag>
    ) : null,
  ].filter(Boolean);
