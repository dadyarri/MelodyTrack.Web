  ...
  return (
    <>
      <PageLayout
          ...
          actions={
            <Space wrap className="schedule-header-actions">
              <ShortcutButton shortcut="←" leadingIcon={<LeftOutlined />} label="Пред." onClick={() => controller.setWeekStart((value) => value.subtract(1, "week"))} />
              <ShortcutButton shortcut="Home" label="Сегодня" onClick={() => controller.setWeekStart(controller.weekStart.startOf("week"))} />
              <ShortcutButton shortcut="→" leadingIcon={<RightOutlined />} label="След." onClick={() => controller.setWeekStart((value) => value.add(1, "week"))} />
        ...
        <section className="schedule-page">
          ...
          <div className="schedule-quick-filters">
            ...
            <Space.Compact className="schedule-quick-filters-controls">
              <UserSelect value={controller.effectiveProviderFilterId} onChange={controller.setProviderFilterId} disabled={controller.isSpecialistFilterLocked} />
              {!controller.isSpecialistFilterLocked && controller.auth.user?.id ? (
                <ShortcutButton
                  ...
                  label="Моё"
                  onClick={() => controller.setProviderFilterId((current) => current === controller.auth.user?.id ? undefined : controller.auth.user?.id)}
                />
              ) : null}
              <Button disabled={controller.isSpecialistFilterLocked || !controller.effectiveProviderFilterId} onClick={() => controller.setProviderFilterId(undefined)}>
                Сбросить
              </Button>
            </Space.Compact>
          </div>
      ...
      <AppointmentDetailsModal
        ...
        onComplete={(appointment) => controller.updateMutation.mutate({ id: appointment.id, input: { isCompleted: true, isCanceled: false }, expectedActivityId: controller.selectedAppointmentBaselineActivityId ?? undefined })}
        onCancel={(appointment) => controller.updateMutation.mutate({ id: appointment.id, input: { isCanceled: true }, expectedActivityId: controller.selectedAppointmentBaselineActivityId ?? undefined })}
        onRestore={(appointment) => controller.updateMutation.mutate({ id: appointment.id, input: { isCompleted: false, isCanceled: false }, expectedActivityId: controller.selectedAppointmentBaselineActivityId ?? undefined })}
        onDelete={(appointment) => {
          if (appointment.recurringRule) {
            controller.setAppointmentToDelete(appointment);
            controller.setAppointmentToDeleteBaselineActivityId(controller.selectedAppointmentBaselineActivityId ?? appointment.lastActivity?.id ?? null);
            return;
          }

          controller.modal.confirm({
            title: "Удалить запись?",
            onOk: () => controller.deleteMutation.mutate({ id: appointment.id, expectedActivityId: controller.selectedAppointmentBaselineActivityId ?? undefined }),
          });
        }}
      />
      <RecurringDeleteModal
        ...
        onDelete={(appointment, scope) => controller.deleteMutation.mutate({ id: appointment.id, scope, expectedActivityId: controller.appointmentToDeleteBaselineActivityId ?? undefined })}
      />
      ...
      <AppointmentEditModal
        ...
        onSubmit={(values) => {
          ...
          controller.editMutation.mutate({ id: controller.appointmentToEdit.id, input: values, expectedActivityId: controller.appointmentToEditBaselineActivityId ?? undefined });
        }}
      />
